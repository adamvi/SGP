				...
				if (is.null(simex.use.my.coefficient.matrices)) {
					for (z in seq_along(sim.iters)) {
						simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]][[z]] <-
							rq.mtx(tmp.gp.iter[1:k], Knots_Boundaries, my.path.knots.boundaries, lam=L, b=z, tmp.dbname, tmp.simex.sample.size, N=tmp.n.size)
					}
					...
					if (toupper(parallel.config[["BACKEND"]]) == "FOREACH") {
							simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]] <-
								foreach(z=iter(sim.iters), .packages=c("quantreg", "data.table", "RSQLite"), 
										.export=c("rq.method", "taus", "content_area.progression", "tmp.slot.gp", "year.progression", "year_lags.progression"),
						...
						if (par.start$par.type == 'MULTICORE') {
							simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]] <- 
								mclapply(sim.iters, function(z) rq.mtx(tmp.gp.iter[1:k], Knots_Boundaries, my.path.knots.boundaries, lam=L, b=z, tmp.dbname, tmp.simex.sample.size, N=tmp.n.size), mc.cores=par.start$workers)
						}
						if (par.start$par.type == 'SNOW') {
							simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]] <-
								parLapply(par.start$internal.cl, sim.iters, function(z) rq.mtx(tmp.gp.iter[1:k], Knots_Boundaries, my.path.knots.boundaries, lam=L, b=z, tmp.dbname, tmp.simex.sample.size, N=tmp.n.size))
						}
					}
				} else {
...
rq.mtx <- function(gp.iter, Knots_Boundaries, my.path.knots.boundaries, lam, b, tmp.dbname, simex.sample.size, N) {
	mod <- character()
	s4Ks <- "Knots=list("
