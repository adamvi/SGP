					...
					if (toupper(parallel.config[["BACKEND"]]) == "FOREACH") {
							simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]] <-
								foreach(z=iter(sim.iters), .packages=c("quantreg", "splines", "data.table", "RSQLite"),
										.export=c("Knots_Boundaries", "rq.method", "taus", "content_area.progression", "tmp.slot.gp", "year.progression", "year_lags.progression", "k"),
						...
						if (par.start$par.type == 'MULTICORE') {
							simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]] <- 
								mclapply(sim.iters, function(z) rq.mtx(tmp.gp.iter[1:k], Knots_Boundaries, my.path.knots.boundaries, L, z, tmp.dbname, tmp.simex.sample.size, tmp.n.size, rq.method, taus, content_area.progression, tmp.slot.gp, year.progression, year_lags.progression, k), mc.cores=par.start$workers)
						}
...
rq.mtx <- function(gp.iter, Knots_Boundaries, my.path.knots.boundaries, lam, b, tmp.dbname, simex.sample.size, N, rq.method, taus, content_area.progression, tmp.slot.gp, year.progression, year_lags.progression, k) {
	mod <- character()
	s4Ks <- "Knots=list("
