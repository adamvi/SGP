				...
				if (is.null(simex.use.my.coefficient.matrices)) {
					for (z in seq_along(sim.iters)) {
						simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]][[z]] <-
							rq.mtx(tmp.gp.iter[1:k], Knots_Boundaries, my.path.knots.boundaries, L, z, tmp.dbname, tmp.simex.sample.size, tmp.n.size, rq.method, taus, content_area.progression, tmp.slot.gp, year.progression, year_lags.progression)
					}
				...
				if (is.null(simex.use.my.coefficient.matrices)) {
					if (toupper(parallel.config[["BACKEND"]]) == "FOREACH") {
							simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]] <-
								foreach(z=iter(sim.iters), .packages=c("quantreg", "data.table", "RSQLite"), 
										.export=c("", "taus", "", ""),
						...
						if (par.start$par.type == 'MULTICORE') {
							simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]] <- 
								mclapply(sim.iters, function(z) rq.mtx(tmp.gp.iter[1:k], Knots_Boundaries, my.path.knots.boundaries, L, z, tmp.dbname, tmp.simex.sample.size, tmp.n.size, rq.method, taus, content_area.progression, tmp.slot.gp, year.progression, year_lags.progression), mc.cores=par.start$workers)
						}
						if (par.start$par.type == 'SNOW') {
							simex.coef.matrices[[paste("qrmatrices", tail(tmp.gp,1), k, sep="_")]][[paste("lambda_", L, sep="")]] <-
								parLapply(par.start$internal.cl, sim.iters, function(z) rq.mtx(tmp.gp.iter[1:k], Knots_Boundaries, my.path.knots.boundaries, L, z, tmp.dbname, tmp.simex.sample.size, tmp.n.size, rq.method, taus, content_area.progression, tmp.slot.gp, year.progression, year_lags.progression))
						}
					}
				} else {
...
rq.mtx <- function(gp.iter, Knots_Boundaries, my.path.knots.boundaries, lam, b, tmp.dbname, simex.sample.size, N, rq.method, taus, content_area.progression, tmp.slot.gp, year.progression, year_lags.progression) {
	mod <- character()
	s4Ks <- "Knots=list("
	s4Bs <- "Boundaries=list("
	for (i in seq_along(gp.iter)) {
		knt <- paste("Knots_Boundaries", my.path.knots.boundaries, "[['Lambda_", lam, "']][['knots_", gp.iter[i], "']]", sep="")
		bnd <- paste("Knots_Boundaries", my.path.knots.boundaries, "[['Lambda_", lam, "']][['boundaries_", gp.iter[i], "']]", sep="")
		mod <- paste(mod, " + bs(prior_", i, ", knots=", knt, ", Boundary.knots=", bnd, ")", sep="")
		s4Ks <- paste(s4Ks, "knots_", gp.iter[i], "=", knt, ",", sep="")
		s4Bs <- paste(s4Bs, "boundaries_", gp.iter[i], "=", bnd, ",", sep="")
	}
	
	if (is.null(simex.sample.size)){
		rqdata <- dbGetQuery(dbConnect(SQLite(), dbname = tmp.dbname), paste("select * from simex_data where b in ('", b, "')", sep=""))
	} else {
